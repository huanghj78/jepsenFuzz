...
const (
	...
	createRecordTableTemplate  = `create table if not exists record (id BIGSERIAL PRIMARY KEY,
...
func (c *bankCase) moveMoney(ctx context.Context, db *sql.DB, delay delayMode) {
	...
	// log.Infof("Transferring %d from account %d to account %d with index %s\n", amount, from, to, index)
	if err != nil {
		return
	}
}
...
func (c *bankCase) tryDrop(db *sql.DB, index int) bool {
	...
	query := fmt.Sprintf("SELECT tablename FROM pg_catalog.pg_tables WHERE tablename LIKE 'accounts%d'", index)
		...
		go func() {
			...
			for {
				...
				for i := 0; i < batchSize; i++ {
					args[i] = fmt.Sprintf("(%d, %d, '%s')", startIndex+i, 1000, remark[:rand.Intn(maxLen)+1])
				}

				// query := fmt.Sprintf("INSERT IGNORE INTO accounts%s (id, balance, remark) VALUES %s", index, strings.Join(args, ","))
				query := fmt.Sprintf("INSERT INTO accounts%s (id, balance, remark) VALUES %s ON DUPLICATE KEY UPDATE NOTHING;", index, strings.Join(args, ","))
