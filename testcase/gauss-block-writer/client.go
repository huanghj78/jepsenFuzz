...
func (c *WriterClient) SetUp(ctx context.Context, _ []cluster.Node, clientNodes []cluster.ClientNode, idx int) error {
	...
	dsn := fmt.Sprintf("postgres://test:test@123@%s/postgres?sslmode=disable&target_session_attrs=read-write", addressesStr)
...
func (bw *blockWriter) batchExecute(db *sql.DB, tableNum int) error {
	...
	for i := 0; i < blockWriterBatchSize; i++ {
		...
		bw.blockCount++
		bw.values[i] = fmt.Sprintf("(%d,'%s',%d,'\\x%s')", blockID, bw.id, bw.blockCount, fmt.Sprintf("%x", blockData))
	}
	var (
		err   error
		index string
