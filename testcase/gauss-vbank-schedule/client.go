...
func (c *Client) genCreateTableSQL(i int) string {
	...
	case PKTypeDecimal:
		pkType += "DECIMAL(10, 2) PRIMARY KEY"
	case PKTypeString:
		pkType += "VARCHAR(64) PRIMARY KEY"
	default:
	...
	balance2 DECIMAL(10, 2) GENERATED ALWAYS AS (-balance) STORED,
	created_at TIMESTAMP,
	updated_at TIMESTAMP
)`
	return fmt.Sprintf(createTableTemplate, c.getTableName(i), pkType)
}

func (c *Client) genCreateIndexSQL1(id int) string {
	return fmt.Sprintf("CREATE INDEX i_balance ON %s (balance)", c.getTableName(id))
}

func (c *Client) genCreateIndexSQL2(id int) string {
	return fmt.Sprintf("CREATE INDEX i_balance2 ON %s (balance2)", c.getTableName(id))
}

func (c *Client) genCreateIndexSQL3(id int) string {
	return fmt.Sprintf("CREATE INDEX i_updated_at ON %s (updated_at)", c.getTableName(id))
}
...
func (c *Client) SetUp(ctx context.Context, _ []cluster.Node, clientNodes []cluster.ClientNode, idx int) error {
	...
	_, err = db.ExecContext(ctx, "CREATE TABLE IF NOT EXISTS v_bank_txn_status (ts BIGINT PRIMARY KEY)")
	for i := 0; i < tblCnt; i++ {
		log.Info(c.genCreateTableSQL(i))
		_, err = db.ExecContext(ctx, c.genCreateTableSQL(i))
		if err != nil {
			log.Error(c.logStr(err))
			return err
		}
		//CREATE INDEX i_balance ON v_bank (balance);
		// CREATE INDEX i_balance2 ON v_bank (balance2);
		// CREATE INDEX i_updated_at ON v_bank (updated_at)
		_, err = db.ExecContext(ctx, c.genCreateIndexSQL1(i))
		if err != nil {
			log.Error(c.logStr(err))
			return err
		}
		_, err = db.ExecContext(ctx, c.genCreateIndexSQL2(i))
		if err != nil {
			log.Error(c.logStr(err))
			return err
		}
		_, err = db.ExecContext(ctx, c.genCreateIndexSQL3(i))
		if err != nil {
			log.Error(c.logStr(err))
			return err
		}
...
func (c *Client) beginTxn(ctx context.Context, reqType VBReqType) (uint64, error) {
	...
	// var ts uint64
	var tempTs float64
	err = tx.QueryRowContext(ctx, "SELECT EXTRACT(EPOCH FROM clock_timestamp()) * 1000000000 AS timestamp_ns").Scan(&tempTs)
	if err != nil {
		return 0, err
	}
	ts := uint64(tempTs)
...
func (c *Client) endTxn(ctx context.Context, resp *Response, err error) {
	if err != nil {
		...
		if resp.ReqType != reqTypeRead {
			_, err = c.tx.ExecContext(ctx, fmt.Sprintf("INSERT INTO v_bank_txn_status (ts) VALUES (%d)", resp.TS))
			if err != nil {
				resp.setErr(err)
				c.tx.Rollback()
				return
...
func (c *Client) invokeReadSingleTable(ctx context.Context) (result *BankState, err error) {
	result = &BankState{}
	rows, err := c.tx.QueryContext(ctx, "SELECT CAST(id AS INTEGER), balance FROM v_bank ORDER BY id")
	if err != nil {
		return
	}
	defer rows.Close()
